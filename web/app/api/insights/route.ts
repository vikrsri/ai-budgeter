...
export async function POST(request:Request){try{
 ...
 const {question,month,source,provider}=payload;
 if(typeof question!=="string"||!question.trim()||question.length>1000||typeof month!=="string"||!(month==="all"||/^\d{4}-(0[1-9]|1[0-2])$/.test(month))||!["all","amex","discover","other"].includes(source)||!["csv","plaid"].includes(provider))throw new HttpError("Choose a period and enter a question of up to 1,000 characters.");
 const all=await readTransactions(owner),rows=all.filter(t=>(t.provider||"csv")===provider&&(source==="all"||t.source===source));
