...
export async function POST(request:Request){try{
 ...
 const legacy=account===undefined&&["all","amex","discover","other"].includes(source);
 if(typeof question!=="string"||!question.trim()||question.length>1000||typeof month!=="string"||!(month==="all"||/^\d{4}-(0[1-9]|1[0-2])$/.test(month))||!["csv","plaid"].includes(provider)||(!legacy&&!isAccountSelection(account,provider)))throw new HttpError("Choose a period and account, and enter a question of up to 1,000 characters.");
