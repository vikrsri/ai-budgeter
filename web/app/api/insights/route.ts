...
import {demoTransactions} from "@/lib/transactions";
...
export async function POST(request:Request){try{
 ...
 const {question,month,demo,source}=payload;
 if(typeof question!=="string"||!question.trim()||question.length>1000||typeof month!=="string"||!(month==="all"||/^\d{4}-(0[1-9]|1[0-2])$/.test(month))||!["all","amex","discover"].includes(source))throw new HttpError("Choose a period and enter a question of up to 1,000 characters.");
 const all=demo===true?demoTransactions():await readTransactions(owner),rows=all.filter(t=>source==="all"||t.source===source);
