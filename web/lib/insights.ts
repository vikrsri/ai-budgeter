...
export function summarize(rows:Transaction[],month:string) {
 ...
 const merchants=new Map<string,number>();selected.filter(t=>t.kind!=="payment").forEach(t=>merchants.set(t.merchant,(merchants.get(t.merchant)||0)+t.amount));
 const months=[...new Set(rows.map(t=>t.date.slice(0,7)))].sort();
 return {period:month,currency:"USD",amountUnit:"cents",transactionCount:selected.length,netSpending:spending(selected),paymentsExcluded:selected.filter(t=>t.kind==="payment").length,refunds:selected.filter(t=>t.kind==="credit").reduce((s,t)=>s+Math.abs(t.amount),0),categories:categoryTotals(selected),merchants:[...merchants].map(([name,amount])=>({name,amount})).sort((a,b)=>b.amount-a.amount).slice(0,100),monthlyTotals:months.map(m=>({month:m,netSpending:spending(rows.filter(t=>t.date.startsWith(m)))})),accounts:["amex","discover"].map(source=>({source,netSpending:spending(selected.filter(t=>t.source===source))})),recurring:detectRecurring(rows).map(({history,...r})=>r),coverage:{from:rows.map(t=>t.date).sort()[0]||null,to:rows.map(t=>t.date).sort().at(-1)||null}};
}
export function instantInsight(rows:Transaction[],question:string,month:string):string {
 ...
 if(!rows.length)return "Import transactions from Amex or Discover to see a spending summary.";
