import {categoryFor,isDiscoverDirectPay,normalizeStoredTransaction,statementMerchant,type Source,type Transaction} from "./transactions";
...
export function normalizePlaid(t:PlaidTransaction,account:PlaidAccount,source:Source):Transaction|null {
 ...
 const merchant=statementMerchant(t.merchant_name||t.name,t.name),detail=t.personal_finance_category?.detailed||"";
 ...
 return normalizeStoredTransaction({id:`plaid:${t.transaction_id}`,date:t.date,merchant,amount:cents,source,category,kind:payment?"payment":cents<0?"credit":"purchase",accountId:account.account_id,accountName:account.name,provider:"plaid",pending:t.pending,currency:"USD"});
}
